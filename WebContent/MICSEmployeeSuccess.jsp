...
<table class="formOutlineOrange">
	...
	<tr>
		...
		<td class="textField">:</td>
		<td><s:select name="usertype" list="userTypes" listKey="id" listValue="name" label="Select Type" width="264" style="width: 264px" disabled="true"/></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		...
		<td colspan="3">
			...
			<s:iterator value="micsUserNumberReq" status="stat" var="numberReq" >
				<tr>
					...
					<td>
						<s:checkbox name="micsUserNumberReq[%{#stat.index}].remove" fieldValue="true" label="Remove" onClick="checkDelete(this,'micsUserNumberReq[%{#stat.index}].mainPublic');" disabled="true"/>
					</td>
					<td>
						<s:property value="%{#numberReq.publicNumber}"/>
						 
						<s:if test='%{#numberReq.publicNumber == #currMainPublic}'>
							<s:checkbox name="micsUserNumberReq[%{#stat.index}].mainPublic" fieldValue="true" value="true" onClick="toggle(this,'N','mainPublic');" label="Set as Main" disabled="true"/>
					...
					<td>
						<s:textfield name="micsUserNumberReq[%{#stat.index}].privateNumber" key="mics.form.privateNumber" size="5" class="fontContent" disabled="true"/>
						<s:if test='%{#numberReq.privateNumber == #currMainPrivate}'>
							<s:checkbox name="micsUserNumberReq[%{#stat.index}].mainPrivate" fieldValue="true" value="true" onClick="toggle(this,'Y','mainPrivate');" label="Set as Main" disabled="true"/>
							<s:text name="mics.form.setAsMain"/>
						</s:if>
						<s:else>
					...
					<td>
						<s:textfield name="micsUserNumberReq[%{#stat.index}].sipuri" key="mics.form.sipuri" size="15" class="fontContent" disabled="true"/>
					</td>
					<td>
						<s:textfield name="micsUserNumberReq[%{#stat.index}].orderIndex" key="mics.form.orderIndex" size="2" class="fontContent" disabled="true"/>
					</td>
					<td>
	...
	<tr>
		...
		<td>
			...
			<s:select name="groupkey" list="groups" listKey="groupid" listValue="groupname" label="Select Group Id" width="264" style="width: 264px" disabled="true"/>
		</td>
		<td>&nbsp;</td>
	</tr>
	<tr>
